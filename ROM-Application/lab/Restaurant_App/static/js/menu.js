...
function getStringItem(name,quantity,cost){
    ...
    s += '<div style="width: 8em; height:1em; text-align: left;">'+name+"</div><p>"+'&nbsp&nbsp&nbsp&nbsp&nbspx'+quantity+'&nbsp&nbsp&nbsp'+total.toString()+"$</p></div>";
    return s;
}
    ...
    $.ajax({
        ...
        success: function(response) {
            ...
            for(let i=0; i<response.length; i++){
                ...
                if(i==0 && response[i]['date'] == currentDate){
                ...
                else if (response[i]['date'] == currentDate){
                ...
                for(let j=0; j<response[i]['items_list'].length; j++){
                    ...
                    if(response[i]['date'] == currentDate){
                        s += tmp;
                    }
...
function getCheckOut(rest_id){
    ...
    $.ajax({
        ...
        success: function(response) {
            ...
            for(let i=0; i<response.length; i++){
                //console.log("order_id "+response[i]['order_id']);
                if(currentDate!=response[i]['date']){
                    continue;
                }
                for(let j=0; j<response[i]['items_list'].length; j++){
